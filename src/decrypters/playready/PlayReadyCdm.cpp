...
bool PlayreadyCdm::Initialize(const DRM::Config& drmConfig, std::string_view decrypterPath)
{
  char* error = nullptr;
  const std::string prdPath = FILESYS::PathCombine(decrypterPath, "device.prd");
  playready_cdm_free(m_cdm);
                                            ...
                                            const std::vector<uint8_t>& kid)
{
  ...
  const std::string response = SendRequestToLicenseServer(
