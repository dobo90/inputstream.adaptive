...
bool WidevineCdm::Initialize(const DRM::Config& drmConfig, std::string_view decrypterPath)
{
  char* error = nullptr;
  const std::string wvdPath = FILESYS::PathCombine(decrypterPath, "device.wvd");
  widevine_cdm_free(m_cdm);
