...
{
  std::vector<std::string> blocks{StringUtils::Split(std::string{licenseUrl}, '|')};
    ...
    widevine_cdm_t* cdm)
{
  ...
  std::unique_ptr<widevine_cdm_license_request_t, decltype(&widevine_cdm_license_request_free)>
      request(widevine_cdm_session_get_license_request(session, wv_pssh, &error),
