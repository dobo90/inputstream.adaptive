...
constexpr std::string_view PROP_MAX_BUFFER_LENGTH = "inputstream.adaptive.max_buffer_length";
...
ADP::KODI_PROPS::CCompKodiProps::CCompKodiProps(const std::map<std::string, std::string>& props)
{
  ...
  for (const auto& prop : props)
  {
    ...
    else if (prop.first == PROP_MAX_BUFFER_LENGTH)
    {
      m_maxBufferLength = STRING::ToUint32(prop.second);
    }
